...
def unique_proteins(stav: str, znak: str, dfs: dict[str: pd.DataFrame]):
    df = dfs[znak]
    fc_col_all = [col for col in df.columns if '_fc' in col]
    # fc_col_zdravi = [col for col in fc_col_all if 'Z' in col]
    # fc_col_chori = [col for col in fc_col_all if 'K' in col or 'C' in col]
    
    col_all = [col for col in df.columns if (('K' in col) or ('Z' in col) or ('C' in col)) and '_' not in col]
    col_zdravi = [col for col in col_all if 'Z' in col]
    col_chori = [col for col in col_all if 'C' in col or 'K' in col]
    
    proteiny_vsetci_zdravi = [df[col] > 0 for col in col_zdravi]
    proteiny_vsetci_chori = [df[col] > 0 for col in col_chori]

    return df.loc[np.bitwise_and.reduce(proteiny_vsetci_zdravi), col_all] if stav == 'z' else df.loc[np.bitwise_and.reduce(proteiny_vsetci_chori), col_all]
...
def calculate_fc(dfs: dict[int | str: pd.DataFrame]):
    ...
    for znak in dfs:
        df = dfs[znak]
        col_all = [col for col in df.columns if 'K' in col or 'Z' in col or 'C' in col]
        col_zdravi = [col for col in col_all if 'Z' in col]
        col_chori = [col for col in col_all if 'C' in col or 'K' in col]

        df['zdr_mean'] = df.loc[:, col_zdravi].mean(axis=1)
        df['chori_mean'] = df.loc[:, col_chori].mean(axis=1)
...
def main():
    for file in os.scandir(PROLINE_RAW_DATA_PATH):
        if file.is_file():
            ...
            # print(dfs['CD19'])
            print(unique_proteins('z', 'CD4', dfs))
